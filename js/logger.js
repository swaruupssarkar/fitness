...
const Logger = (() => {
  ...
  function renderLogger(el, plan, prefillDate) {
    ...
    el.innerHTML = `
      ...
      <div class="log-date-row">
        <label class="log-date-label" for="log-bodyweight">Today's Body Weight (kg)</label>
        ...
        <div class="sets-table">
          ...
          ${ex.sets.map((set, si) => `
            <div class="set-row" data-ei="${ei}" data-si="${si}">
              <span class="drag-handle set-drag" title="Drag to reorder set">⠿</span>
  ...
  function bindLogDragDrop(list, rebind) {
    ...
    list.querySelectorAll('.exercise-card').forEach(card => {
      ...
      card.addEventListener('dragend', () => {
        ...
        list.querySelectorAll('.exercise-card').forEach(c => c.classList.remove('drag-over'));
      });

      card.addEventListener('dragstart', e => {
        if (!card.draggable) { e.preventDefault(); return; }
        dragSrcEi = +card.dataset.ei;
        dragSrcSi = null;
      ...
      card.addEventListener('drop', e => {
        e.preventDefault();
        if (dragSrcSi !== null) return;
        const tgtEi = +card.dataset.ei;
        if (dragSrcEi === null || dragSrcEi === tgtEi) return;
        const [moved] = logExercises.splice(dragSrcEi, 1);
    ...
    list.querySelectorAll('.set-row').forEach(row => {
      ...
      row.addEventListener('dragend', () => {
        ...
        list.querySelectorAll('.set-row').forEach(r => r.classList.remove('drag-over'));
      });

      row.addEventListener('dragstart', e => {
        if (!row.draggable) { e.preventDefault(); return; }
        dragSrcEi = +row.dataset.ei;
        dragSrcSi = +row.dataset.si;
        e.dataTransfer.effectAllowed = 'move';
        e.stopPropagation();
      ...
      row.addEventListener('drop', e => {
        ...
        e.stopPropagation();
        if (dragSrcSi === null) return;
        const tgtEi = +row.dataset.ei;
        const tgtSi = +row.dataset.si;
        if (dragSrcEi !== tgtEi || dragSrcSi === tgtSi) return;
        const [moved] = logExercises[tgtEi].sets.splice(dragSrcSi, 1);
  ...
  function openForDate(date, planDayName = null) {
    ...
    // If a plan day name is given, auto-select it and skip the picker
    if (planDayName) {
      const day = plan.days.find(d => d.name === planDayName);
      if (day) {
        selectedDay  = day;
        logExercises = day.exercises.map(ex => ({
          name: ex.name,
        ...
        _presetDate = null;
        renderLogger(el, plan, date);
        App.showView('log', true);
        return;
      }
    }

    // No plan day known — show the day picker
    selectedDay = null;
    renderDayPicker(el, plan);
    App.showView('log', true);
  }

  return { render, editLog, openForDate };
})();
