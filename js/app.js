...
const App = (() => {
  ...
  function _calPlannedName(cellDate, dow, session, helpers) {
    ...
    if (Object.keys(scheduleMap).length > 0) {
      // Blank out everything before the plan start date
      if (planStart && cellDate < planStart) return null;
        ...
        <div class="month-cell${session ? ' cal-active' : ''}${missed ? ' cal-missed' : ''}${planned ? ' cal-planned' : ''}${isToday ? ' cal-today' : ''}">
          ...
          ${logs.length
            ...
            : date <= todayStr && !beforeStart
              ? `<button class="cal-edit-btn cal-log-btn" data-log-date="${date}" data-plan-day="${plannedDayName || ''}">${EDIT_SVG} Log</button>`
              : ''}
        </div>`;
    }
  ...
  function buildWeekCalendar(plan, todayStr) {
    ...
    for (let i = 0; i < 7; i++) {
      ...
      // Is this cell before the plan's start date?
      const beforeStart = helpers.planStart && cellDate < helpers.planStart;
          ...
          <div class="wday-action">
            ${logs.length
              ...
              : beforeStart
                ? ''
                : isPast
                  ? `<button class="wday-log-btn wday-log-past-btn" data-log-date="${date}" data-plan-day="${plannedDayName || ''}">+ Log Past</button>`
                  : isToday
                    ? `<button class="wday-log-btn wday-log-past-btn" data-log-date="${date}" data-plan-day="${plannedDayName || ''}">+ Log</button>`
                    : ''}
          </div>
        </div>`;
    }
  ...
  function bindCalendarEvents() {
    document.querySelectorAll('.cal-edit-btn:not(.cal-log-btn)').forEach(btn => btn.addEventListener('click', (e) => {
      e.stopPropagation();
      Logger.editLog(btn.dataset.id);
    }));
    document.querySelectorAll('.cal-log-btn').forEach(btn => btn.addEventListener('click', (e) => {
      e.stopPropagation();
      Logger.openForDate(btn.dataset.logDate, btn.dataset.planDay || null);
    }));
  ...
  function renderHistory() {
    ...
    el.innerHTML = `
      <div class="view-header">
        <h1>History</h1>
        <p class="subtitle">${logs.length} workout${logs.length !== 1 ? 's' : ''}</p>
      </div>
      ${logs.length === 0 ? `
