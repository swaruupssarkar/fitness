...
const App = (() => {
  ...
  function buildMonthCalendar(plan, todayStr) {
    ...
    for (let d = 1; d <= daysInMonth; d++) {
      ...
      const planned = !session ? (scheduleMap[dow] || null) : null;
      const isToday = date === todayStr;
      cells += `
        <div class="month-cell${session ? ' cal-active' : ''}${planned ? ' cal-planned' : ''}${isToday ? ' cal-today' : ''}">
          <span class="month-day-num${isToday ? ' today-num' : ''}">${d}</span>
          ${session ? `<div class="month-session">${session}</div>` : ''}
          ${planned ? `<div class="month-planned">${planned}</div>` : ''}
  ...
  function renderDashboard() {
    ...
    el.innerHTML = `
      <div class="view-header">
        <h1 class="greeting-heading">${getGreeting()}, Swaruup 👋</h1>
      ...
      <div class="metrics-grid">
        ...
        <div class="metric-card">
          ...
          <div class="metric-label">🏋️ ${topLift ? topLift.name : 'Top Lift This Week'}</div>
        </div>
        <div class="metric-card">
    ...
    Auth.onAuthReady(async (user) => {
      if (!user) {
        ...
        document.getElementById('btn-google-signin').addEventListener('click', async () => {
          try {
            document.getElementById('btn-google-signin').textContent = 'Signing in…';
            await Auth.signInWithGoogle();
          } catch (err) {
            console.error(err);
            document.getElementById('btn-google-signin').innerHTML =
      ...
      // Load / migrate data from Firestore
  ...
  return { navigate, showView, toast, init };
})();
