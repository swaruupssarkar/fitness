...
const App = (() => {
  ...
  const calState  = { year: today0.getFullYear(), month: today0.getMonth() };
  ...
  function buildMonthCalendar(plan, todayStr) {
    ...
    for (let d = 1; d <= daysInMonth; d++) {
      ...
      const isScheduled = !session && !!scheduleMap[dow];
      const missed   = isScheduled && date < todayStr;   // past scheduled, no log
      const planned  = isScheduled && date >= todayStr;  // today/future scheduled, no log
      cells += `
        <div class="month-cell${session ? ' cal-active' : ''}${missed ? ' cal-missed' : ''}${planned ? ' cal-planned' : ''}${isToday ? ' cal-today' : ''}">
          ...
          ${missed  ? `<div class="month-missed-label">${scheduleMap[dow]}</div>` : ''}
          ${planned ? `<div class="month-planned">${scheduleMap[dow]}</div>` : ''}
          ${logs.length ? `<button class="cal-edit-btn" data-id="${logs[0].id}"><svg width="10" height="10" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2.5" stroke-linecap="round" stroke-linejoin="round"><path d="M11 4H4a2 2 0 0 0-2 2v14a2 2 0 0 0 2 2h14a2 2 0 0 0 2-2v-7"/><path d="M18.5 2.5a2.121 2.121 0 0 1 3 3L12 15l-4 1 1-4 9.5-9.5z"/></svg></button>` : ''}
        </div>`;
    }
  ...
  function renderDashboard() {
    ...
    const streak   = Metrics.getStreak();
    ...
    el.innerHTML = `
      ...
      <div class="metrics-grid">
        <div class="metric-card accent">
          <div class="metric-icon">
            <svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="1.8" stroke-linecap="round" stroke-linejoin="round"><path d="M12 2c0 0-4 4-4 8a4 4 0 0 0 8 0c0-4-4-8-4-8z"/><path d="M12 14c0 0-2 1.5-2 3a2 2 0 0 0 4 0c0-1.5-2-3-2-3z"/></svg>
          </div>
      ...
      <div class="cta-section">
        ${doneToday
          ...
          : `<div class="cta-row">
               <a href="#log" class="btn btn-primary btn-lg">Log Today's Workout →</a>
               <button class="btn btn-outline" id="btn-edit-log-toggle"><svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><path d="M11 4H4a2 2 0 0 0-2 2v14a2 2 0 0 0 2 2h14a2 2 0 0 0 2-2v-7"/><path d="M18.5 2.5a2.121 2.121 0 0 1 3 3L12 15l-4 1 1-4 9.5-9.5z"/></svg> Edit Log</button>
             </div>`
        }
  ...
  function bindPlanEvents(el) {
    ...
    document.getElementById('btn-save-edit').addEventListener('click', () => {
      ...
      Storage.upsertPlan({ ...existing, name, days: editDays.filter(d => d.name.trim()), isCustom: true });
      toast('Plan updated!');
  ...
  function renderHistory() {
    ...
    el.innerHTML = `
      <div class="view-header">
        <h1>History</h1>
        <p class="subtitle">${logs.length} workout${logs.length !== 1 ? 's' : ''} logged</p>
      </div>
      ${logs.length === 0 ? `
