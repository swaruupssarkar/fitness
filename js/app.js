...
const App = (() => {
  ...
  function renderHistory(fromDate = '', toDate = '') {
    ...
    function applyFilter() {
      ...
      clearBtn.classList.toggle('hidden', !from && !to);
      renderHistory(from, to);
    }

    // 'input' fires on every picker selection (mobile); 'change' fires on blur (desktop)
    histFrom.addEventListener('change', applyFilter);
    histFrom.addEventListener('input',  applyFilter);
    histTo.addEventListener('change',   applyFilter);
    histTo.addEventListener('input',    applyFilter);
